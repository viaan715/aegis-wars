...
export function drawVehicle(v){
  ...
  if(v.cat==='helo'){
    ...
    // Cosmetic decal (player tank only, no stat effect)
    if(v.decal==='stripe'){ctx.strokeStyle='rgba(232,216,128,0.85)';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(-v.w/2,v.h/2-3);ctx.lineTo(v.w/2,-v.h/2+3);ctx.stroke();}
    else if(v.decal&&v.decal!=='none'){const dd=getDecal(v.decal);if(dd&&dd.glyph){ctx.fillStyle='rgba(232,216,128,0.9)';ctx.font='8px Courier New';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(dd.glyph,0,0);}}
  }
  ctx.restore();
  ...
  if(!v.dead){
    // Health bar + name label are drawn in true screen-pixel space (not
    // world space) so they stay a constant on-screen size and offset
    // regardless of the active zoom (gunner scope mode) or shake transform.
    const tf=ctx.getTransform();
    const scale=Math.hypot(tf.a,tf.b);
    const pt=tf.transformPoint(new DOMPoint(v.x,v.y));
    const vh=v.h*scale;
    ctx.save();ctx.setTransform(1,0,0,1,0,0);
    const bw=Math.max(v.w,18)+8,pct=v.crew/v.maxCrew;
    ctx.fillStyle='#080a05';ctx.fillRect(pt.x-bw/2,pt.y-vh-(isAir?20:12),bw,4);
    ctx.fillStyle=v.team==='blue'?'#3a7ade':pct>0.6?'#3a8a30':pct>0.3?'#c8a030':'#c03030';
    ctx.fillRect(pt.x-bw/2,pt.y-vh-(isAir?20:12),bw*pct,4);
    ctx.fillStyle='rgba(0,0,0,0.6)';ctx.fillRect(pt.x-28,pt.y-vh-(isAir?32:26),56,10);
    ctx.fillStyle=isAir?'#7ecfb3':v.team==='blue'?'#6aaaf0':'#8a8a60';
    ctx.font='7px Courier New';ctx.textAlign='center';ctx.fillText((v.tankName||v.name||'').slice(0,12),pt.x,pt.y-vh-(isAir?24:18));
    ctx.restore();
...
// Buildings/bunkers crumble across their hp stages (intact -> cracked ->
// heavily damaged -> rubble pile) instead of flipping straight from solid
// to destroyed. hp starts at TERRAIN_MAX_HP (see scene.js).
function drawCrumblingStructure(t,intactCol,destroyedCol,windowCol){
  if(t.hp<=0){
    ctx.fillStyle=destroyedCol;
    ctx.beginPath();
    ctx.moveTo(-t.w/2,t.h/2-2);ctx.lineTo(-t.w/3,-t.h/4);ctx.lineTo(-t.w/8,t.h/3);
    ctx.lineTo(t.w/6,-t.h/5);ctx.lineTo(t.w/2.2,t.h/2-3);ctx.lineTo(t.w/2,t.h/2);
    ctx.lineTo(-t.w/2,t.h/2);ctx.closePath();ctx.fill();
    ctx.strokeStyle='#1c1a10';ctx.lineWidth=0.5;ctx.stroke();
    return;
  }
  ctx.fillStyle=intactCol;ctx.fillRect(-t.w/2,-t.h/2,t.w,t.h);
  ctx.strokeStyle='#282018';ctx.lineWidth=1;ctx.strokeRect(-t.w/2,-t.h/2,t.w,t.h);
  if(windowCol){ctx.fillStyle=windowCol;ctx.fillRect(-t.w/2+4,-t.h/2+4,8,10);ctx.fillRect(t.w/2-12,-t.h/2+4,8,10);}
  if(t.hp<3){
    ctx.strokeStyle='rgba(10,10,8,0.65)';ctx.lineWidth=0.8;
    ctx.beginPath();ctx.moveTo(-t.w/4,-t.h/2);ctx.lineTo(-t.w/6,t.h/2);ctx.stroke();
  }
  if(t.hp<2){
    ctx.fillStyle='#1c1a10';ctx.fillRect(t.w/2-10,-t.h/2,10,8);
    ctx.beginPath();ctx.moveTo(t.w/5,-t.h/2);ctx.lineTo(t.w/3,t.h/2);ctx.stroke();
  }
}

export function drawTerrain(){G.terrain.forEach(t=>{ctx.save();ctx.translate(t.x,t.y);switch(t.type){case 'rock':case 'snowrock':ctx.fillStyle=t.type==='snowrock'?'#5a6068':'#5a5a48';ctx.beginPath();ctx.ellipse(0,0,t.w/2,t.h/2,0.2,0,Math.PI*2);ctx.fill();ctx.fillStyle=t.type==='snowrock'?'#8a9098':'#6a6a58';ctx.beginPath();ctx.ellipse(-2,-3,t.w/3,t.h/3,-0.1,0,Math.PI*2);ctx.fill();break;case 'bunker':drawCrumblingStructure(t,'#3a3828','#1e1e14',null);break;case 'building':drawCrumblingStructure(t,'#342e22','#1e1c14','#282018');break;case 'rubble':ctx.fillStyle='#2e2c1e';ctx.beginPath();ctx.moveTo(-t.w/2,t.h/2);ctx.lineTo(0,-t.h/2);ctx.lineTo(t.w/2,t.h/2);ctx.closePath();ctx.fill();break;case 'tree':ctx.fillStyle=G.fogOfWar?'#0e2008':'#1a3010';ctx.beginPath();ctx.arc(0,-2,8,0,Math.PI*2);ctx.fill();ctx.fillStyle=G.fogOfWar?'#082004':'#112808';ctx.fillRect(-2,4,4,8);break;}ctx.restore();});}
...
export function drawWeather(){
  ...
  const windAngle=Math.atan2(G.weather.windY,G.weather.windX);
  const fallAngle=windAngle*0.3+Math.PI/2.3; // mostly downward, drifting with the wind
  ctx.save();
  ctx.strokeStyle='rgba(180,200,220,0.25)';
  ctx.lineWidth=1;
  for(let i=0;i<60;i++){
    const seed=i*53.7;
    const x=((seed*7+t*260)%(W+80))-40;
    const y=((seed*13+t*420)%(H+60))-30;
    ctx.beginPath();
    ctx.moveTo(x,y);
    ctx.lineTo(x+Math.cos(fallAngle)*10,y+Math.sin(fallAngle)*10);
    ctx.stroke();
  }
  ctx.restore();
}
...
export function drawAmmoCrates(){
  const bob=Math.sin(Date.now()/300)*2;
  G.ammoCrates.forEach(c=>{
    ctx.save();ctx.translate(c.x,c.y+bob);
    ctx.fillStyle='#5a4a18';ctx.fillRect(-8,-6,16,12);
    ctx.strokeStyle='#e8d870';ctx.lineWidth=1.2;ctx.strokeRect(-8,-6,16,12);
    ctx.strokeStyle='#c8b870';ctx.lineWidth=0.8;
    ctx.beginPath();ctx.moveTo(-8,0);ctx.lineTo(8,0);ctx.moveTo(0,-6);ctx.lineTo(0,6);ctx.stroke();
    ctx.restore();
  });
}
...
function drawCockpitOverlay(){
  // Vignette frame representing the gunner's sight housing
  ctx.save();
  const grd=ctx.createRadialGradient(W/2,H/2,90,W/2,H/2,W/2);
  grd.addColorStop(0,'rgba(0,0,0,0)');grd.addColorStop(1,'rgba(0,0,0,0.78)');
  ctx.fillStyle=grd;ctx.fillRect(0,0,W,H);
  // Interior bezel with rivets
  ctx.strokeStyle='rgba(10,10,8,0.9)';ctx.lineWidth=26;ctx.strokeRect(13,13,W-26,H-26);
  ctx.fillStyle='rgba(40,38,24,0.9)';
  for(let i=20;i<W-20;i+=34){ctx.beginPath();ctx.arc(i,14,2,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.arc(i,H-14,2,0,Math.PI*2);ctx.fill();}
  for(let i=20;i<H-20;i+=34){ctx.beginPath();ctx.arc(14,i,2,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.arc(W-14,i,2,0,Math.PI*2);ctx.fill();}
  // Optic housing markings etched on the lens (fixed to the sight, not the
  // aim point -- the real point of impact is the moving crosshair drawn
  // with the world, same as top-down mode)
  const cx=W/2,cy=H/2;
  ctx.strokeStyle='rgba(90,186,80,0.85)';ctx.lineWidth=1;
  ctx.beginPath();ctx.arc(cx,cy,60,0,Math.PI*2);ctx.stroke();
  ctx.beginPath();ctx.moveTo(cx-90,cy);ctx.lineTo(cx-20,cy);ctx.moveTo(cx+20,cy);ctx.lineTo(cx+90,cy);ctx.stroke();
  ctx.beginPath();ctx.moveTo(cx,cy-90);ctx.lineTo(cx,cy-20);ctx.moveTo(cx,cy+20);ctx.lineTo(cx,cy+90);ctx.stroke();
  // Range stadia ticks (mock rangefinder marks)
  for(let i=-3;i<=3;i++){if(i===0)continue;const tx=cx+i*30;ctx.beginPath();ctx.moveTo(tx,cy-92);ctx.lineTo(tx,cy-84);ctx.stroke();ctx.beginPath();ctx.moveTo(tx,cy+84);ctx.lineTo(tx,cy+92);ctx.stroke();}
  ctx.fillStyle='rgba(90,186,80,0.85)';ctx.font='8px Courier New';ctx.textAlign='left';
  ctx.fillText('GUNNER SCOPE · '+COCKPIT_ZOOM.toFixed(1)+'x',24,H-24);
  ctx.textAlign='right';ctx.fillText((G.p1&&G.p1.tankName||'').toUpperCase(),W-24,H-24);
  ctx.restore();
}

export function draw(){
  const off=shakeOffset();
  const cockpit=G.viewMode==='cockpit'&&G.p1&&!G.p1.dead;
  ctx.save();
  if(cockpit){ctx.translate(W/2,H/2);ctx.scale(COCKPIT_ZOOM,COCKPIT_ZOOM);ctx.translate(-G.p1.x,-G.p1.y);}
  ...
  drawCaptureZones();drawTerrain();drawAmmoCrates();
  ...
  drawFog();
  drawWeather();
  if(G.phase==='playing'&&G.p1&&!G.p1.dead&&!cockpit){
    const a=G.p1.tAngle;
  ...
  if(cockpit&&G.phase==='playing'){
    ctx.strokeStyle='rgba(90,186,80,0.9)';ctx.lineWidth=1/COCKPIT_ZOOM;
    ctx.beginPath();ctx.moveTo(G.mouseX-6/COCKPIT_ZOOM,G.mouseY);ctx.lineTo(G.mouseX+6/COCKPIT_ZOOM,G.mouseY);ctx.stroke();
    ctx.beginPath();ctx.moveTo(G.mouseX,G.mouseY-6/COCKPIT_ZOOM);ctx.lineTo(G.mouseX,G.mouseY+6/COCKPIT_ZOOM);ctx.stroke();
  }
  ctx.restore();
  if(cockpit)drawCockpitOverlay();
