...
async function loadResponses(formId) {
  const responses = await queryAll('SELECT * FROM responses WHERE form_id = $1 ORDER BY submitted_at DESC', [formId]);
  return Promise.all(
    responses.map(async (r) => {
      const answers = await queryAll('SELECT * FROM answers WHERE response_id = $1', [r.id]);
      return {
        id: r.id,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json({ questions: await questionsForForm(form.id), responses: await loadResponses(form.id) });
  })
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const responses = await loadResponses(form.id);
